...
/// A join waits on NetworkManager (up to 30 s daemon-side) before the manager
/// replies, so the deadline sits above that.
pub(crate) const WIFI_CMD_TIMEOUT: std::time::Duration = std::time::Duration::from_secs(45);
