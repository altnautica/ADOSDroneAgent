...
//! `params` (typed payload codecs), `bank` (the live channel values),
//! `link` (the lane state machine), `transport` (serial + TX/RX tasks),
//! `cmdsock` (the command socket), `sidecar` (the state file), `config` +
//! `paths` (wiring).
