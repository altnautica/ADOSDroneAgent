...
pub fn default_config_yaml(
    ...
    let network = regulatory_block(region, ack_at);
    format!(
...
/// Build the `network` config block carrying the operating-region posture
/// (pure). Default is unrestricted: the radio transmits without pinning a
/// country, and the operator is responsible for local radio rules (the note is
/// written honestly, it never claims compliance). A pinned region records the
/// country plus an install-time acknowledgement stamp. Only `network.regulatory`
/// is set here; the agent deep-merges the rest of the network defaults.
pub fn regulatory_block(region: Option<&str>, ack_at: &str) -> String {
    match region {
        Some(code) => format!(
            "network:\n  \
# Operating region for the long-range radio. A region is pinned, so the radio\n  \
# applies this country's channels and power limits before it transmits.\n  \
regulatory:\n    \
mode: \"region\"\n    \
region: \"{code}\"\n    \
ack_operator: \"installer\"\n    \
ack_at: \"{ack_at}\"\n"
        ),
        None => "network:\n  \
# Operating region for the long-range radio. Default is unrestricted: the radio\n  \
# transmits on your channel at the device's power budget without pinning a\n  \
# country. You are responsible for the radio rules where you fly. Pin a region\n  \
# here or in the app to apply that country's channel and power limits.\n  \
regulatory:\n    \
mode: \"unrestricted\"\n    \
region: null\n    \
ack_operator: null\n    \
...
fn write_default_config(
    ...
    // Stamp the region acknowledgement only when a region is actually pinned.
    let ack_at = if region.is_some() {
        crate::result::now_iso8601_utc()
    } else {
        String::new()
    };
...
mod tests {
    ...
    fn default_config_carries_identity_and_blocks() {
        ...
        assert!(cfg.contains("region: null"));
    }
    ...
    fn default_config_empty_fc_port_is_blank_string() {
        ...
        // Cloud (reach-from-anywhere) server mode + a pinned region.
        assert!(cfg.contains("mode: \"cloud\""));
        assert!(cfg.contains("mode: \"region\""));
        assert!(cfg.contains("region: \"US\""));
        assert!(cfg.contains("ack_at: \"2026-07-04T00:00:00Z\""));
    }

    #[test]
    fn regulatory_block_defaults_to_unrestricted_and_pins_on_demand() {
        let free = regulatory_block(None, "");
        assert!(free.contains("mode: \"unrestricted\""));
        assert!(free.contains("region: null"));
        // The default note is honest: it never claims compliance.
        assert!(free.to_lowercase().contains("responsible"));
        assert!(!free.to_lowercase().contains("compliant"));

        let pinned = regulatory_block(Some("IN"), "2026-07-04T10:00:00Z");
        assert!(pinned.contains("mode: \"region\""));
        assert!(pinned.contains("region: \"IN\""));
        assert!(pinned.contains("ack_operator: \"installer\""));
        assert!(pinned.contains("ack_at: \"2026-07-04T10:00:00Z\""));
    }
