...
//!
//! This step does NOT provision a panel overlay, in either direction of a
//! `--display <id>` choice. It used to, and that was a brick: it wrote a
//! hard-coded `dtoverlay=waveshare35a` and commented out `vc4-kms-v3d` for ANY
//! explicit `--display` value on a ground station, with no presence detection
//! and no probation marker — so `--display hdmi_touch_xpt2046` (a real
//! board-declared id) killed HDMI with nothing to self-heal it, and it ran 2nd
//! in the graph, long before any hardware probe. Every boot-config overlay edit
//! belongs to `scripts/drivers/install-display-overlay.sh`, which is panel-id
//! aware, presence aware and probation-arming; `config_identity` delegates to
//! it. What remains here is the REVERT direction only: stripping a stale SPI
//! panel overlay and re-enabling the GPU KMS overlay, which can only restore
//! HDMI, never take it away.
...
impl Step for PurgeResidue {
    ...
    fn run(&self, ctx: &mut Ctx) -> StepOutcome {
        purge_orphan_route();
        // Only the REVERT direction lives here. Provisioning a panel overlay
        // needs presence detection and a probation marker to be brick-safe,
        // and this step runs 2nd in the graph with neither — so every forward
        // edit is `install-display-overlay.sh`'s, via `config_identity`.
        match lcd_residue_action(ctx) {
            LcdAction::Revert => revert_lcd_boot_config(),
            // The shell auto-detector owns the boot config for `auto` and for
            // an explicitly selected panel on a ground station (bound-SPI keep
            // / HDMI clean / probation), so we neither strip a real panel nor
            // break HDMI.
            LcdAction::Defer => {}
        }
        StepOutcome::Ok
...
/// Decide the SPI-LCD residue action from the operator's `--display` choice,
/// mirroring `config_identity::provision_overlays`' resolution: an explicit
/// `--display` wins; otherwise a ground station defaults to `auto` (panel
/// auto-detected) and every other profile to `none`. Pure + unit-tested.
///
...
///   * an explicit panel id -> Defer on a ground station (the panel IS its UI
///     and the shell installer is the only thing that may write its overlay),
///     Revert on any other profile (a drone must never let the SPI framebuffer
///     overlay steal fb0 from the GPU).
fn lcd_residue_action(ctx: &Ctx) -> LcdAction {
    ...
    match resolved.as_str() {
        ...
        _ if ctx.profile == "ground_station" => LcdAction::Defer,
        _ => LcdAction::Revert,
    }
}
...
/// (b) Revert the SPI-LCD residue in the Pi boot config, keep-first
/// snapshotting through [`crate::steps::boot_config`] so the display probe's
/// pristine baseline survives. No-op when the config is absent or the revert
/// produces no change (idempotent).
fn revert_lcd_boot_config() {
    match boot_config::edit_boot_config("SPI-LCD residue revert", revert_lcd_config) {
        BootConfigEdit::Written(cfg) => {
            tracing::info!(cfg = %cfg.display(), "reverted SPI-LCD boot config residue")
        }
        BootConfigEdit::Unchanged | BootConfigEdit::Absent => {}
        BootConfigEdit::Refused(why) => {
            tracing::warn!(reason = %why, "SPI-LCD residue revert skipped")
        }
    }
}
...
mod tests {
    ...
    // ── lcd_residue_action: the display-choice-aware decision ──
    ...
    fn action_gs_display_none_reverts_so_hdmi_boots() {
        // A ground station installed with --display none (HDMI cockpit) must
        // NOT keep an SPI-LCD overlay that would steal fb0 from the GPU.
        assert_eq!(
            lcd_residue_action(&ctx_with(Some("none"), "ground_station")),
            LcdAction::Revert
        );
    }
    ...
    fn action_gs_display_auto_defers_to_shell_autodetect() {
        ...
        assert_eq!(
            lcd_residue_action(&ctx_with(Some("auto"), "ground_station")),
            LcdAction::Defer
        );
        // Absent --display on a GS resolves to "auto" -> Defer (never strips a
        // real SPI-LCD GS to a white screen).
        assert_eq!(
            lcd_residue_action(&ctx_with(None, "ground_station")),
            LcdAction::Defer
        );
    }

    #[test]
    fn action_gs_explicit_panel_never_writes_a_boot_config_here() {
        // The brick this step used to cause: ANY explicit --display on a GS
        // wrote a hard-coded waveshare overlay and disabled KMS, with no
        // presence detection and no probation marker — so a real board-declared
        // id like hdmi_touch_xpt2046 killed HDMI with nothing to self-heal it.
        // The panel-id-aware, presence-aware shell installer owns that edit.
        for id in ["waveshare35a", "hdmi_touch_xpt2046"] {
            assert_eq!(
                lcd_residue_action(&ctx_with(Some(id), "ground_station")),
                LcdAction::Defer,
                "--display {id} must not be provisioned from this step"
            );
        }
    }
    ...
    fn action_drone_reverts_to_keep_the_gpu() {
        ...
        assert_eq!(
            lcd_residue_action(&ctx_with(None, "drone")),
            LcdAction::Revert
        );
        assert_eq!(
            lcd_residue_action(&ctx_with(Some("none"), "drone")),
            LcdAction::Revert
        );
        // Even an explicit panel id on a drone reverts (a drone is not a panel host).
        assert_eq!(
            lcd_residue_action(&ctx_with(Some("waveshare35a"), "drone")),
            LcdAction::Revert
        );
    }
}
