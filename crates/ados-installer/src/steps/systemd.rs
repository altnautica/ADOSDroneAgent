...
/// total lockup) to also catch the cases the watchdog cannot — a hung kernel
/// task while PID 1 still pets the watchdog, or an oops. `hung_task_timeout` is
/// the kernel default (120 s) so a legitimately busy I/O path never false-trips.
...
# watchdog) and the trace survives in the persistent journal. Generated by\n\
# the agent; do not edit by hand.\n\
kernel.panic = 10\n\
...
mod tests {
    ...
    #[test]
    fn resilience_sysctl_reboots_on_fault_with_safe_hung_task_timeout() {
        ...
        assert!(body.contains("kernel.hung_task_panic = 1"));
        // Kernel default (120s) so a busy I/O path never false-trips a reboot.
