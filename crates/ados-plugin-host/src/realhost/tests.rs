...
async fn compute_offload_routes_a_job_through_the_host_to_the_node() {
    ...
    let host =
        RealHost::new().with_compute(Arc::new(ComputeClient::new(format!("http://{addr}"), None)));
    ...
    let submitted = route_host_method(&host, Method::ComputeJobSubmit, "p", &submit_args, &caps)
        .await
        .unwrap();
    ...
    let outputs_reply = route_host_method(&host, Method::ComputeJobOutputs, "p", &out_args, &caps)
        .await
        .unwrap();
...
/// A host whose offload-link sidecar names a paired node at `target`, so
/// `execution: auto` resolves to offload. The sidecar path is returned so the
/// test can remove it.
fn offload_host(tag: &str, target: &str) -> (RealHost, PathBuf) {
    use ados_protocol::offload_link::{write_offload_link_to, OffloadLink};
    let sidecar = temp_sidecar(tag);
    let link = OffloadLink::stamped(
        true,
        true,
...
fn open_args(session_id: &str) -> Value {
    map(&[
        ("execution", Value::from("auto")),
        ("session_id", Value::from(session_id)),
    ])
}

#[tokio::test]
async fn reopening_another_plugins_offload_session_is_refused() {
    let (host, sidecar) = offload_host("reopen-foreign", "127.0.0.1:18093");
    host.compute_stream_open("a", &open_args("shared"))
        .await
        .unwrap();
    let err = err_body(host.compute_stream_open("b", &open_args("shared")).await);
    assert!(err.contains("held by another plugin"), "{err}");
    // The session stays with its opener.
    assert_eq!(
        host.offload_streams.lock().unwrap()["shared"].plugin_id,
        "a"
    );
    host.release_session("a", 0).await;
    std::fs::remove_file(&sidecar).ok();
}

#[tokio::test]
async fn reopening_a_session_whose_lane_ended_starts_a_fresh_one() {
    let (host, sidecar) = offload_host("reopen-dead", "127.0.0.1:18094");
    host.compute_stream_open("p", &open_args("s"))
        .await
        .unwrap();
    // The lane dies (a panic, or anything else that ends its task).
    host.offload_streams.lock().unwrap()["s"].task.abort();
    for _ in 0..100 {
        if host.offload_streams.lock().unwrap()["s"].task.is_finished() {
            break;
        }
        tokio::time::sleep(std::time::Duration::from_millis(10)).await;
    }
    let reopened = host
        .compute_stream_open("p", &open_args("s"))
        .await
        .unwrap();
    // A fresh lane, not the dead one reported as already open.
    assert_eq!(map_get(&reopened, "already_open"), None);
    assert_eq!(
        map_get(&reopened, "opened").and_then(Value::as_bool),
        Some(true)
    );
    assert!(!host.offload_streams.lock().unwrap()["s"].task.is_finished());
    host.release_session("p", 0).await;
    std::fs::remove_file(&sidecar).ok();
}

#[tokio::test]
async fn an_offload_lane_is_restarted_after_its_orchestrator_ends() {
    // A node that accepts and hangs up: each orchestrator run fails its submit
    // and ends. The session is still open, so the lane must come back.
    let node = tokio::net::TcpListener::bind("127.0.0.1:0").await.unwrap();
    let target = node.local_addr().unwrap().to_string();
    let (host, sidecar) = offload_host("restart", &target);
    host.compute_stream_open("p", &open_args("s"))
        .await
        .unwrap();
    let attempts = async {
        for _ in 0..2 {
            let (conn, _) = node.accept().await.unwrap();
            drop(conn);
        }
    };
    tokio::time::timeout(RECONNECT_INTERVAL * 3, attempts)
        .await
        .expect("the lane must submit again after its orchestrator ended");
    host.release_session("p", 0).await;
    std::fs::remove_file(&sidecar).ok();
}

#[test]
fn offload_frame_size_and_budget_are_clamped() {
    // Absent: the defaults.
    assert_eq!(
        offload_frame_and_budget(&map(&[])),
        (
            OFFLOAD_DEFAULT_WIDTH,
            OFFLOAD_DEFAULT_HEIGHT,
            OFFLOAD_DEFAULT_BUDGET_MS
        )
    );
    // A budget of days would keep a lock alive on a stalled link; a frame of
    // billions of pixels is a node allocation. Both are bounded.
    let huge = map(&[
        ("width", Value::from(1_i64 << 40)),
        ("height", Value::from(1_i64 << 40)),
        ("target_budget_ms", Value::from(1_000_000_000_000_i64)),
    ]);
    assert_eq!(
        offload_frame_and_budget(&huge),
        (
            OFFLOAD_MAX_DIMENSION as u32,
            OFFLOAD_MAX_DIMENSION as u32,
            OFFLOAD_MAX_BUDGET_MS
        )
    );
    let tiny = map(&[
        ("width", Value::from(1)),
        ("height", Value::from(1)),
        ("target_budget_ms", Value::from(1)),
    ]);
    assert_eq!(
        offload_frame_and_budget(&tiny),
        (
            OFFLOAD_MIN_DIMENSION as u32,
            OFFLOAD_MIN_DIMENSION as u32,
            OFFLOAD_MIN_BUDGET_MS
        )
    );
    // An in-range request is taken as given.
    let camera = map(&[
        ("width", Value::from(1920)),
        ("height", Value::from(1080)),
        ("target_budget_ms", Value::from(500)),
    ]);
    assert_eq!(offload_frame_and_budget(&camera), (1920, 1080, 500));
}
