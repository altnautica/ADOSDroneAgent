...
/// its own task from the service main loop. Removes a stale socket first, chmods
/// it 0o660, and hands group ownership to the `ados` group so a non-root
/// operator (the API service) can write it. Returns only on a bind error; the
            ...
            Ok((stream, _addr)) => {
                ...
                tokio::spawn(async move {
                    if let Err(e) = handle_conn(stream, state).await {
                        tracing::debug!(error = %e, "radio command conn error");
                    }
                });
            }
            Err(e) => {
                tracing::warn!(error = %e, "radio command accept failed");
...
/// terminated response. Bounded read so a client that never sends a newline
