...
# The LAN-front marker the installer reconciles on every upgrade. There is no
# operator toggle for it any more: `ados rust {enable,disable} front` was
# retired when the last packaged ground-station network routes went native, so
# "FastAPI reclaims the LAN port" stopped being true — the residual Python
# surface no longer serves the routes an operator would be falling back TO. The
# marker itself stays: it is the installer's unit-reconcile mechanism
# (`ados-installer`'s `ensure_front_default_on`, which writes it on every
# install) and the control surface's port selector, not an operator-facing
# choice.
_CONTROL_MARKER = "control-rust-enabled"
_CONTROL_BIN = "/opt/ados/bin/ados-control"
_FRONT_MARKER = "front-rust-enabled"

_FLIP_NAMES = _SVC_NAMES
