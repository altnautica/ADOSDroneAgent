...
out-of-tree products and external OEM partners declare their
peripherals as manifests. The agent loads those manifests from two
sources: Python entry points under the group ``ados.peripherals`` and
YAML files in ``/etc/ados/peripherals/*.yaml``. Each manifest describes
how to match a device on a transport (USB, serial, network, BLE),
