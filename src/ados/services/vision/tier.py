...
the drone yet, so they are conservatively ``False`` here: a board with an NPU
reads ``local`` and a board without one reads ``none`` (never a fabricated
...
def perception_tier(
    *,
    ...
    """
    if has_accelerator and models_fit_locally:
        return "local"
